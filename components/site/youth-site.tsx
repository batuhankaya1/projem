...
import { ArrowUpRight, ArrowRight, ArrowLeft, ArrowUp, Menu, X, MoveDown, Plus } from 'lucide-react';
...
function LivingLine({ step, progress, c, lang }: { step: number; progress: number; c: Copy; lang: Language }) {
  ...
  return <div className={`living-art art-step-${step}`} style={{ '--journey-progress': progress } as CSSProperties} role="img" aria-label={asset.src ? asset.alt[lang] : `${c.graphic} ${scene.period}`}>
    ...
    <div className="art-top"><span>{c.archive}</span><span>№ 00{step+1}</span></div>
    ...
    <div className="archive-notes">{c.scenes.map((item, i) => { const opacity = Math.max(0, 1 - Math.abs(rawStep - i) * 1.7); return <div key={i} className="archive-note" aria-hidden={i !== step} style={{ opacity, filter: `blur(${(1-opacity) * 2}px)`, transform: `translate3d(${(i - rawStep) * 34}px, ${(i - rawStep) * 22}px, 0) rotate(${[-7,4,-3,5,-4][i]}deg) scale(${.96 + Math.max(0, 1 - Math.abs(rawStep - i)) * .04})` }}><span className="archive-pin" /><span className="note-number">{String(i+1).padStart(2,'0')} / {item.stamp}</span><p>{item.note}</p><div className="note-bottom"><span>{['01','02','03'].includes(item.year) ? '2007 →' : item.year}</span><Mark /></div></div>})}</div>
    <span className="origin-label"><span>+</span> {c.origin}</span><span className="future-label">{c.future} <ArrowUpRight size={13}/></span>
    <p className="art-caption">{asset.src ? c.archive : c.visualNote}</p>
  </div>;
}
...
function Story({ c, lang }: { c: Copy; lang: Language }) {
  ...
  const toContent = () => { const el = document.getElementById('content'); if (el) { el.focus({ preventScroll: true }); el.scrollIntoView({ behavior: window.matchMedia('(prefers-reduced-motion: reduce)').matches ? 'instant' : 'smooth' }); } };
  ...
  useEffect(() => {
    const key = (event: KeyboardEvent) => {
      ...
      if (event.key === 'Escape') { toContent(); return; }
      if (!dir) return;
      event.preventDefault();
      const current = Math.round(progressRef.current * 4);
      if (current === 4 && dir > 0) toContent(); else scrollToStep(Math.max(0, Math.min(4, current + dir)));
    };
  ...
  return <section ref={journey} className="story-journey" aria-label={c.storyLabel}>
    <div ref={frame} className="story story-frame" style={{ '--journey-progress': progress } as CSSProperties}>
      <div className="story-topline"><Eyebrow><span className="small-line" />{c.since}</Eyebrow><span>{c.journey}</span><button className="text-link story-skip" onClick={toContent}>{c.skipStory}<ArrowUpRight size={14}/></button></div>
      <div className="story-stage">
        <div className="story-copy-stack">{c.scenes.map((scene, i) => { const opacity = Math.max(0, 1 - Math.abs(rawStep - i) * 1.7); return <div className="story-copy" key={i} aria-hidden={i !== step} style={{ opacity, filter: `blur(${(1-opacity) * 3}px)`, transform: `translate3d(0, ${(i - rawStep) * 34}px, 0) scale(${.97 + opacity * .03})`, pointerEvents: i === step ? 'auto' : 'none' }}>
          ...
          <div aria-live={i === step ? 'polite' : 'off'} aria-atomic="true"><h1>{scene.title.map((line,lineIndex) => <span key={lineIndex} className={lineIndex === 2 ? 'last-line' : ''}>{line}</span>)}</h1><p className="story-body">{scene.body}</p>{'placeholder' in scene && <p className="milestone-placeholder">{scene.placeholder}</p>}</div>
          <div className="story-actions"><Button className="primary-action" onClick={() => i === 4 ? toContent() : scrollToStep(i+1)}>{i === 0 ? c.discover : i === 4 ? c.today : c.next}<ArrowUpRight /></Button><button className="text-link content-shortcut" onClick={toContent}>{c.skip}<ArrowRight size={15}/></button></div>
        </div>})}</div>
        <LivingLine step={step} progress={progress} c={c} lang={lang}/>
      </div>
      <div className="story-bottom"><div className="scroll-hint"><MoveDown size={22}/><span>{c.scrollHint}</span></div><nav className="story-progress" aria-label={c.storyLabel}>{c.scenes.map((s,i)=><button key={i} aria-label={`${c.sceneLabel} ${i+1}: ${s.label}`} aria-current={step === i ? 'step' : undefined} onClick={()=>scrollToStep(i)}><span className="step-line"><span style={{ transform: `scaleX(${Math.max(0, Math.min(1, rawStep - i))})` }} /></span><span className="step-year">{i === 0 ? '2007' : i === 4 ? s.year : `0${i}`}</span></button>)}</nav><div className="step-controls"><span aria-hidden="true"><strong>0{step+1}</strong><span> / 05</span></span><Button variant="ghost" className="round-button" size="icon" disabled={step === 0} onClick={()=>scrollToStep(step-1)} aria-label={c.previous}><ArrowLeft /></Button><Button variant="ghost" className="round-button" size="icon" onClick={()=>step === 4 ? toContent() : scrollToStep(step+1)} aria-label={step === 4 ? c.today : c.next}><ArrowRight /></Button></div></div>
    </div>
...
function ExternalPost({ post, allowed, c, number }: { post: Copy['social'][number]; allowed: boolean; c: Copy; number: number }) {
  const ref = useRef<HTMLElement>(null); const [visible,setVisible]=useState(false); const [loaded,setLoaded]=useState(false); const [failed,setFailed]=useState(false);
  const safeEmbed = /^https:\/\/(www\.)?instagram\.com\/p\/[A-Za-z0-9_-]+\/embed\/?$/.test(post.embedUrl) || /^https:\/\/platform\.twitter\.com\/embed\/Tweet\.html\?id=\d+$/.test(post.embedUrl);
  const safeUrl = /^https:\/\/(www\.)?(instagram\.com|x\.com|twitter\.com)\//.test(post.url);
  useEffect(()=> { if(!ref.current) return; const observer = new IntersectionObserver(([entry])=> { if(entry.isIntersecting) {setVisible(true); observer.disconnect();} },{rootMargin:'100px'}); observer.observe(ref.current); return ()=>observer.disconnect(); },[]);
  useEffect(()=> {setLoaded(false); setFailed(false);},[allowed,post.embedUrl]);
  useEffect(()=> {if(!allowed || !visible || !safeEmbed || loaded || failed) return; const t=setTimeout(()=>setFailed(true),8000); return ()=>clearTimeout(t);},[allowed,visible,safeEmbed,loaded,failed]);
  return <article className={`social-item social-item-${number}`} ref={ref}><div className="social-platform"><span>{post.platform === 'Instagram' ? <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" aria-hidden="true"><rect x="3" y="3" width="18" height="18" rx="5"/><circle cx="12" cy="12" r="4"/><circle cx="17.5" cy="6.5" r=".7" fill="currentColor"/></svg> : <span className="x-logo">𝕏</span>}{post.platform}</span><span>{c.samplePost}</span></div><div className="social-visual"><span className="social-index">0{number+1}</span><h3>{post.title}</h3><span className="social-line" aria-hidden="true"/></div><p>{post.body}</p>{allowed && visible && safeEmbed && !failed && <iframe loading="lazy" src={post.embedUrl} title={`${post.platform}: ${post.title.replace(/\n/g,' ')}`} referrerPolicy="no-referrer" sandbox="allow-scripts allow-same-origin allow-popups" onLoad={()=>{setLoaded(true);setFailed(false);}} onError={()=>setFailed(true)} className={loaded ? 'social-embed loaded' : 'social-embed'} />}{allowed && safeEmbed && failed && !loaded && <p role="status">{c.embedFail}</p>}{safeUrl ? <a className="text-link" href={post.url} target="_blank" rel="noreferrer noopener">{c.viewPost}<ArrowUpRight size={15}/></a> : <span className="post-pending">{c.postPending}<Plus size={14}/></span>}</article>;
}
function HomeContent({ c, lang, allowed, toggleConsent }: { c: Copy; lang: Language; allowed: boolean; toggleConsent: ()=>void }) {
  return <>
    <section className="about-section section-pad" id="content" tabIndex={-1} aria-labelledby="about-title"><div><Eyebrow>{c.aboutEyebrow}</Eyebrow><h2 id="about-title">{c.aboutTitle}</h2></div><div className="about-summary"><span className="section-index" aria-hidden="true">01 — 05</span><p>{c.aboutBody}</p><Link className="text-link" href={`/${lang}/about`}>{c.aboutMore}<ArrowUpRight size={18}/></Link><p className="draft-note">{c.draft}</p></div></section>
    <Program c={c} lang={lang}/>
    <section className="impact-section section-pad"><Eyebrow>{c.impactEyebrow}</Eyebrow><h2>{c.impactTitle}</h2><div className="impact-grid">{c.metrics.map(m=><div key={m.label}><strong>{m.value}</strong><h3>{m.label}</h3><p>{m.note}</p></div>)}</div></section>
    <section className="social-section section-pad"><div className="section-heading"><div><Eyebrow>{c.socialEyebrow}</Eyebrow><h2>{c.socialTitle}</h2></div><p>{c.socialIntro}</p></div><div className="social-grid">{c.social.map((post,i)=><ExternalPost post={post} allowed={allowed} c={c} number={i} key={i}/>)}</div><div className="social-privacy"><p>{c.socialPrivacy}</p><button className="text-link" onClick={toggleConsent}>{allowed ? c.socialRevoke : c.socialConsent}<Plus size={14}/></button></div></section>
  </>;
}
...
export function YouthSite({ lang, page }: { lang: Language; page?: PageKey }) {
  ...
  const [allowed,setAllowed]=useState(false); const [privacy,setPrivacy]=useState(false);
  useEffect(()=> { document.documentElement.lang=lang; try {setAllowed(localStorage.getItem('living-line-social')==='allowed');}catch{} },[lang]);
