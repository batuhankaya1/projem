      ...
      <Sheet open={menu} onOpenChange={setMenu}>
        ...
        <SheetContent side="top" className="route-menu-sheet" showCloseButton={false}>
          ...
          <div className="corporate-index">
            ...
            <nav className="index-navigation" aria-label={lang === 'tr' ? 'Ana menü' : 'Main navigation'}><Link className="index-row" href={`/${lang}`} onClick={() => setMenu(false)} aria-current={!page ? 'page' : undefined} style={{ '--index-delay': '0ms' } as CSSProperties}><span className="index-number">00</span><span className="index-label"><strong>{c.home}</strong><small>{lang === 'tr' ? 'Hikâyenin başlangıcı' : 'The beginning of the story'}</small></span><ArrowUpRight aria-hidden="true"/></Link>{menuPageKeys.map((key, i) => { const copyIndex = pageKeys.indexOf(key); return <Link key={key} className="index-row" href={`/${lang}/${key}`} onClick={() => setMenu(false)} aria-current={page === key ? 'page' : undefined} style={{ '--index-delay': `${(i + 1) * 55}ms` } as CSSProperties}><span className="index-number">0{i+1}</span><span className="index-label"><strong>{c.nav[copyIndex]}</strong><small>{menuNotes[i]}</small></span><ArrowUpRight aria-hidden="true"/></Link> })}</nav>
          </div>
